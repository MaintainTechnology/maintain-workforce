...
## `public/design-system/graphics/web/`
Optimized copies (flattened onto their brand backdrop, resized, JPEG) that the site and the
design-system `index.html` paint — the deployed payload stays ~2 MB instead of ~124 MB. The
full-resolution and transparent-PNG originals in THIS folder are the real assets to hand to
designers; this folder lives outside `public/` on purpose, so the masters never ship to the
public CDN.
