...
# Full-res sources live here (repo-only, never deployed); the optimized output
# ships from public/ so the site serves ~2MB instead of ~124MB of masters.
G = os.path.dirname(os.path.abspath(__file__))
WEB = os.path.normpath(os.path.join(G, "..", "..", "public", "design-system", "graphics", "web"))
os.makedirs(WEB, exist_ok=True)

D = (7, 39, 45)      # ink-teal backdrop for dark graphics
...
JOBS = [
    ...
    ("amber-gradient.png", "amber-gradient", 1400, D),
...
# clean stale files from earlier runs (audits-era outputs)
for f in os.listdir(WEB):
    if f.endswith(".png") or f in ("green-gradient.jpg", "mountain-line.jpg"):
